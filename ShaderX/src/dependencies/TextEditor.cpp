...
void TextEditor::Render(bool fullscreen = false)
{
	static int invisible = -1;
	if (fullscreen) invisible = 12;
	else invisible = -1;
	...
	for (int i = 0; i < (int)PaletteIndex::Max; ++i) {
		...
		if (i != invisible) color.w *= ImGui::GetStyle().Alpha;
		else color.w *= 0.0f;
...
void TextEditor::Render(const char *aTitle, bool fullscreen = false, const ImVec2 &aSize, bool aBorder)
{
	mWithinRender = true;
	mTextChanged = false;
	mCursorPositionChanged = false;
...
const TextEditor::Palette &TextEditor::GetFullscreenPallete()
{
	const static Palette p = { {
			0xff00ffff,	// None
			0xffffff00,	// Keyword	
...
const TextEditor::LanguageDefinition &TextEditor::LanguageDefinition::GLSL()
{
	...
	if (!inited) {
		static const char *const keywords[] = {
				"auto", "break", "bool", "case", "char", "const", "continue", "default", "do", "double", "else", "enum", "extern", "float", "for", "goto", "if", "inline", "int", "long", "register", "restrict", "return", "short",
				"signed", "sizeof", "static", "struct", "switch", "typedef", "union", "unsigned", "void", "volatile", "while", "_Alignas", "_Alignof", "_Atomic", "_Bool", "_Complex", "_Generic", "_Imaginary",
				"_Noreturn", "_Static_assert", "_Thread_local", "vec2", "vec3", "vec4", "mat2", "mat3", "mat4", "bvec2", "bvec3", "bvec4", "ivec2", "ivec3", "ivec4", "sampler2D", "samplerCube", "uniform", "varying"
		};
		for (auto &k : keywords)
			langDef.mKeywords.insert(k);
		...
		static const char *const identifiers[] = {
			...
			"radians", "degrees", "sin", "cos", "tan", "exp", "exp2", "inversesqrt", "sign", "fract", "mod", "min", "max", "clamp", "mix", "step",
			"smoothstep", "length", "distance", "dot", "cross", "normalize", "faceforward", "reflect", "refract", "matrixCompMult", "lessThan",
			"lessThanEqual", "greaterThan", "greaterThanEqual", "equal", "notEqual", "any", "all", "not", "texture2D", "textureCube"
		};
		for (auto &k : identifiers) {
			Identifier id;
			id.mDeclaration = "Built-in function";
